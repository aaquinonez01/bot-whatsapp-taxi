...
export class RequestService {
  ...
  async createTaxiRequest(data: CreateTaxiRequestData): Promise<ServiceResponse<TaxiRequest>> {
    try {
      // Validaciones
      ...
      // Verificar si el cliente ya tiene una solicitud PENDING
      const existingRequest = await this.getClientPendingRequest(data.clientPhone)
      if (existingRequest.success && existingRequest.data) {
        return { 
          success: false, 
          error: 'Ya tienes una solicitud de taxi pendiente. Espera a que sea asignada o cancélala primero.' 
        }
      }
  ...
  async assignFirstAvailableDriver(requestId: string, driverPhone: string): Promise<AssignmentResult> {
    try {
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        const driverResult = await this.driverService.getDriverByPhone(driverPhone)
  ...
  async getClientPendingRequest(clientPhone: string): Promise<ServiceResponse<TaxiRequest>> {
    try {
  ...
  async cancelRequest(requestId: string, reason?: string): Promise<ServiceResponse<TaxiRequest>> {
    try {
  ...
  async completeRequest(requestId: string): Promise<ServiceResponse<TaxiRequest>> {
    try {
      const request = await prisma.taxiRequest.update({
        where: { id: requestId },
        data: {
  ...
  async getRequestsByFilters(filters: RequestFilters): Promise<ServiceResponse<TaxiRequest[]>> {
    try {
      const where: any = {}
      ...
      if (filters.clientPhone) {
        where.clientPhone = ValidationUtils.cleanPhoneNumber(filters.clientPhone)
      }
  ...
  async getRequestStats(): Promise<ServiceResponse<RequestStats>> {
    try {
      const [total, pending, assigned, completed, cancelled] = await Promise.all([
        ...
        prisma.taxiRequest.count({ where: { status: RequestStatus.PENDING } }),
        prisma.taxiRequest.count({ where: { status: RequestStatus.ASSIGNED } }),
        prisma.taxiRequest.count({ where: { status: RequestStatus.COMPLETED } }),
        prisma.taxiRequest.count({ where: { status: RequestStatus.CANCELLED } })
      ])
