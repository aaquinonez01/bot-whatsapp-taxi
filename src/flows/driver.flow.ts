...
export const driverAcceptFlow = addKeyword<BaileysProvider, MemoryDB>([
  'acepto', 'acepto', 'si', 'sí', 'ok', 'vale', 'listo', 'tomo', 'yo'
])
  .addAction(async (ctx, { flowDynamic }) => {
    try {
      ...
      if (!driverResult.success || !driverResult.data) {
        ...
        await flowDynamic('📝 Para registrarte como conductor, contacta al administrador: +57 300 123 4567')
        return
      }
      ...
      if (!driver.isActive) {
        await flowDynamic('⚠️ Tu cuenta de conductor está inactiva. Contacta al administrador.')
        return
      }
      ...
      const clientNotificationSuccess = await notificationService.notifyClientAssignment(
        assignedRequest, 
        assignedDriver
      )
      ...
      if (clientNotificationSuccess) {
        ...
        await flowDynamic('⚠️ Error notificando al cliente. Contacta manualmente.')
      }

      // Notificar a todos los otros conductores que la carrera fue tomada
      const otherDriversResult = await notificationService.notifyOtherDriversCareerTaken(
        request.id,
        driverPhone,
        driver.name
      )
...
export const driverRejectFlow = addKeyword<BaileysProvider, MemoryDB>([
  'no', 'rechazar', 'rechazo', 'ocupado', 'no puedo', 'imposible', 'muy lejos'
])
  .addAction(async (ctx, { flowDynamic }) => {
    try {
      const driverPhone = ctx.from
      ...
      await flowDynamic('📝 Carrera rechazada. Será asignada a otro conductor disponible.')
      
      console.log(`Driver ${driverResult.data.name} (${driverPhone}) rejected a ride`)
...
export const driverRegisterFlow = addKeyword<BaileysProvider, MemoryDB>([
  'registrar conductor', 'nuevo conductor', 'register driver'
])
  .addAnswer(
  ...
  .addAction(
    ...
    async (ctx, { flowDynamic, state }) => {
      ...
      try {
        const driverLocation = location.toLowerCase() === 'skip' ? undefined : location
        ...
        if (createResult.success) {
          ...
          await flowDynamic(`📱 Teléfono: ${ValidationUtils.cleanPhoneNumber(driverPhone)}`)
          if (driverLocation) {
...
export const driverStatusFlow = addKeyword<BaileysProvider, MemoryDB>([
  'activo', 'inactivo', 'disponible', 'no disponible', 'conectar', 'desconectar'
])
  .addAction(async (ctx, { flowDynamic }) => {
    try {
      const driverPhone = ctx.from
      ...
      const updateResult = await driverService.updateDriverStatus(driverPhone, isActivating)
...
export const driverLocationFlow = addKeyword<BaileysProvider, MemoryDB>([
  'ubicacion', 'ubicación', 'location', 'donde estoy'
])
  .addAnswer(
    ...
    async (ctx, { fallBack, flowDynamic }) => {
      try {
        ...
        const updateResult = await driverService.updateDriverLocation(driverPhone, newLocation)
...
export const driverInfoFlow = addKeyword<BaileysProvider, MemoryDB>([
  'mi info', 'mi informacion', 'mis datos', 'perfil'
])
  .addAction(async (ctx, { flowDynamic }) => {
    try {
      const driverPhone = ctx.from
      ...
      await flowDynamic(`📅 Registrado: ${driver.createdAt.toLocaleDateString()}`)
