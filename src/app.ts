...
const main = async () => {
    try {
        ...
        setupAPIEndpoints(adapterProvider, handleCtx, driverService, requestService, notificationService)
...
function setupAPIEndpoints(
    ...
    provider.server.post('/v1/messages', handleCtx(async (bot: any, req: any, res: any) => {
        try {
    ...
    provider.server.post('/v1/driver/register', handleCtx(async (bot: any, req: any, res: any) => {
        try {
            const { name, phone, plate, location } = req.body
            const result = await driverService.createDriver({ name, phone, plate, location })
    ...
    provider.server.get('/v1/stats', handleCtx(async (bot: any, req: any, res: any) => {
        try {
    ...
    provider.server.get('/v1/drivers', handleCtx(async (bot: any, req: any, res: any) => {
        try {
    ...
    provider.server.post('/v1/blacklist', handleCtx(async (bot: any, req: any, res: any) => {
        try {
...
// Configurar tareas programadas
function setupScheduledTasks(requestService: RequestService, notificationService: NotificationService) {
    ...
    setInterval(async () => {
        try {
            ...
            if (result.success && result.data && result.data > 0) {
                console.log(`🧹 Limpieza automática: ${result.data} solicitudes expiradas canceladas`)
            }
        } catch (error) {
    ...
    console.log(`✅ Tareas programadas configuradas (limpieza cada ${config.taxi.cleanupIntervalMinutes} min)`)
}
